...
def distribute_recursively(chocolates_list, students_list, index=0, distribution=None):
    ...
    if index < len(students_list) and index < len(chocolates_list):  # checks if there are more students and chocolates
        ...
        distribute_recursively(chocolates_list, students_list, index + 1,
                               distribution)  # calls function recursively for the next student and chocolate
    return distribution
...
iterative_distribution = distribute_iteratively(chocolates_list,
                                                students_list)  # distributing chocolates to students iteratively
recursive_distribution = distribute_recursively(chocolates_list,
                                                students_list)  # distributing chocolates to students recursively
...
def sort_weight(chocolates_list):  # define function, take chocolates list as input
    ...
    sorted_chocolates_types = [chocolate.type for chocolate in
                               chocolates_list]  # iterate over sorted list and get the 'type' attribute
    return sorted_chocolates_types  # return the list of chocolates types sorted by weight


# sorting chocolates_list based on their weights
sorted_chocolates_weight = sort_weight(chocolates_list)
...
def sort_price(chocolates_list):  # define function, take chocolates list as input
    for i in range(1, len(chocolates_list)):  # for loop iterates from second to last element
        ...
        while position >= 0 and float(chocolates_list[position].price[:-4]) > float(current_chocolate.price[
                                                                                    :-4]):  # compares prices after converting to float and removing 'AED' since it's string
            chocolates_list[position + 1] = chocolates_list[position]  # moves chocolate at position to the right
            position -= 1  # decrements position to move to the previous chocolate in the list
        # insert current chocolate into the correct position
        chocolates_list[position + 1] = current_chocolate
    ...
    sorted_chocolates_types = [chocolate.type for chocolate in
                               chocolates_list]  # creates list that has types of chocolates sorted by price and iterates over sorted list and get the 'type' attribute
    return sorted_chocolates_types  # return the list of chocolates types sorted by price


# sorting chocolates_list based on their prices
sorted_chocolates_price = sort_price(chocolates_list)
...
def find_student_price(chocolates_list, price):
    for chocolate in chocolates_list:  # for loop iterates over each chocolate in chocolates_list
        if chocolate.price == price:  # checks if price of current chocolate is equal to the specified price
            return chocolate.student  # returns student who is holding the chocolate with the specified price
...
def find_student_weight(chocolates_list, weight):
    for chocolate in chocolates_list:  # for loop iterates over each chocolate in chocolates_list
        if chocolate.weight == weight:  # checks if weight of current chocolate is equal to the specified weight
            return chocolate.student  # returns student who is holding the chocolate with the specified weight


price_search = '10 AED'  # sets the price to search for to '10 AED'
weight_search = '6 gm'  # sets the weight to search for to '6 gm'

student_by_price = find_student_price(chocolates_list,
                                      price_search)  # calls the find_student_price function with the chocolates_list and price_search as arguments, and assigns the result to the variable student_by_price
student_by_weight = find_student_weight(chocolates_list,
                                        weight_search)  # calls the find_student_weight function with the chocolates_list and weight_search as arguments, and assigns the result to the variable student_by_weight
print("The student that is holding a chocolate with the price", price_search, "is",
      student_by_price)  # prints the student holding a chocolate with the specified price
print("The student that is holding a chocolate with the weight", weight_search, "is",
      student_by_weight)  # prints the student holding a chocolate with the specified weight